...
export default function LiveGiftWallet({ streamId }: LiveGiftWalletProps) {
    ...
    useEffect(() => {
        getStreamGiftTotalAction(streamId).then(({ total, count }) => { setTotal(total); setCount(count) })
    }, [streamId])

    // Fires when a fan sends a gift (the fan-side gift action will trigger this event).
    useEffect(() => {
        const pusher  = getPusherClient()
        const channel = pusher.subscribe(`stream-${streamId}`)
        channel.bind("gift-sent", (g: GiftEvent) => {
