// components/fan/feed/CommentPanel.tsx
"use client"
...
import { SidePanel }           from "@/component/fan/ui/SidePanel"
import "@/styles/fan/CommentPanel.scss"
...
const CommentPanelContent = ({ postId, onCommentAdded, onTotalChange }: ContentProps) => {
    ...
    const handleSubmit = () => {
        ...
        startTransition(async () => {
            ...
            // Reload to pick up the new comment with its user info attached
            const data = await getCommentsAction({ postId, page: 1 })
            setComments(data.comments as Comment[])
            setPages(data.pages)
            setPage(1)
    ...
    return (
        <>
            ...
            <div className="side-panel__scroll" ref={listRef}>
                {loading ? (
                    ...
                    <>
                        ...
                        {page < pages && (
                            <button
                                type="button"
                                className="comment-panel__load-more"
                                onClick={loadMore}
            ...
            <div className="side-panel__footer">
                ...
                {error && <p className="comment-panel__error">{error}</p>}
...
export function CommentPanel({ postId, currentUserId, onClose, onCommentAdded }: Props) {
    // Lives here so the count can render in the panel header
    ...
    return (
        <SidePanel
            ...
            title={<>Comments {total > 0 && <span>({total})</span>}</>}
        >
            <CommentPanelContent
                postId={postId}
